...
class JournalViewModel: ObservableObject {
    ...
    func authenticate(username: String, password: String, isRegister: Bool) async throws {
        ...
        do {
            ...
            let errResponse = (error as? NetworkResponseError)
            if let jsonData = errResponse?.responseBody.data(using: .utf8) {
                let decoder = JSONDecoder()
                let errModel = try decoder.decode(ErrorResponse.self, from: jsonData)
                let error = NSError(domain: "", code: errResponse?.statusCode ?? 400, userInfo: [NSLocalizedDescriptionKey: errModel.detail])
                throw error
            }
        }
