...
struct AuthView: View {
    ...
    var body: some View {
        Form {
            ...
            // Display error from ViewModel if there is one
            if !vm.Error.isEmpty {
                Text(vm.Error)
                    .foregroundColor(.red)
                    .font(.caption)
...
//                        let data = try await NetworkManager.shared.fetchData(method: "GET", urlString: Constant.BASE_URL)
//                        print(data)
                    }
                },
                label: {
    ...
    private func logIn() async {
        ...
        do {
            ...
            try await vm.authenticate(username: username, password: password, isRegister: false)
...
//            try await journalService.register(username: username, password: password)
            try await vm.authenticate(username: username, password: password, isRegister: true)
        } catch {
            self.error = error
        }
        isLoading = false
